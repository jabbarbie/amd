<?php
	...
	$bulan 		= $_GET['bulan'];
	$asuransi 	= ubahasuransi($_GET['asuransi']);
	$tahun 		= $_GET['tahun'];
	// Inisisaliasi Total
	$grandtotaljasa		= 0;
	$grandtotalpart		= 0;
	...
	if ($asuransi <> "4"){
		...
		if ($asuransi == ''){
				$query = "SELECT * FROM unit u JOIN estimasi e ON u.u_kode = e.u_kode WHERE MONTH(es_tgl_masuk) = '$bulan' AND YEAR(es_tgl_masuk) = '$tahun' ORDER BY e.es_tgl_masuk DESC";
		}
		else
			{
				// Jika asuransi dipilih
				$query = "SELECT * FROM unit u JOIN estimasi e ON u.u_kode = e.u_kode WHERE asuransi ='$asuransi' AND month(es_tgl_masuk) = '$bulan' AND YEAR(es_tgl_masuk) = '$tahun' ORDER BY e.es_tgl_masuk DESC";
			}
	}
	else {
				$query = "SELECT * FROM unit u JOIN estimasi e ON u.u_kode = e.u_kode WHERE e.asuransi = 4 ORDER BY e.es_tgl_masuk DESC";	
		}
...
?>
	...
	<div id="wrapper2">
	  <table width="100%" align="center" cellpadding="3" cellspacing="1">
		  <tr><th colspan="8"><h3>Data OutStanding Kendaraan <?php echo ($_GET['asuransi'] != 6) ? ('Asuransi '.ubahasuransi($asuransi)): '';
		  echo ' Bulan '.ubahbulan($_GET['bulan']).' '.$_GET['tahun'] ?> </h3></th></tr>
